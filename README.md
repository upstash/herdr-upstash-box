...
Keys can come from the environment Herdr runs in, or from a `secrets.json` in the plugin config directory:
...
| `agentArgs`           | `[]`                        | Extra arguments appended to the harness command in TUI mode.                                                                                                                                  |
...
| `providerApiKeyEnv`   | `null`                      | Override which variable carries the provider key. By default it follows the model prefix.                                                                                                     |
