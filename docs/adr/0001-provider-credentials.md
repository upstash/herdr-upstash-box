...
- `setup` offers the subscription option first, and defaults to it whenever a token is already present or nothing is configured yet.
- `start-codex` and `start-opencode` drop `providerApiKeyEnv` for the launch, and reconnect and fork ignore it for any mapping it does not fit: another harness, or another provider (a box created on an `openrouter/` model before the config moved to a subscription token). A name outside the standard set is the user's own override and is kept when the harness matches. `launchEnv` additionally refuses to send a subscription token to OpenRouter at all, so no path can turn one into a bearer for the wrong service. Without that second half, a reconnect would have read the Claude token into `OPENAI_API_KEY`, which is exactly the silent wrong-account case decision 2 rules out.
- Setup runs `claude setup-token` under `script`, which gives it a pseudo-terminal widened to 1,024 columns: the command needs a terminal for its browser flow, and it hard-wraps the token at the terminal width, which is what truncated pasted tokens twice. The token is masked in the streamed output and never shown, capture stops the command the moment a token appears so nothing later can reach the screen, and a token the terminal still managed to wrap is joined back from its continuation line rather than printed. `script` ships with macOS and Linux, so this adds no dependency. Paste stays as the fallback, with bracketed paste enabled so a wrapped copy still arrives whole. Every token, captured or pasted, is checked against Anthropic's read-only model listing before it is saved; an outright rejection stops setup, anything else is saved with a note.
