...
- `start-codex` and `start-opencode` drop `providerApiKeyEnv` for the launch, and reconnect and fork ignore it for any mapping whose harness differs from the configured one, since a variable named for Claude Code is wrong for another harness. Without that second half, a reconnect would have read the Claude token into `OPENAI_API_KEY`, which is exactly the silent wrong-account case decision 2 rules out.
