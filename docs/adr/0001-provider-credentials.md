...
- Setup never runs `claude setup-token` itself: with inherited stdio the token would be printed into the popup, and reading it back through a PTY bridge is more machinery than it is worth. The user pastes it, unechoed, and setup checks its shape.
